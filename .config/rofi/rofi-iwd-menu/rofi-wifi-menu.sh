...
CHENTRY=$(echo -e "$MENU" | uniq -u | rofi -dmenu -p "WiFi SSID" -lines "$LINE_COUNT" -a "$HIGHLINE" -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH" -normal-window -steal-focus)
CHSSID=$(echo "$CHENTRY" | sed  's/\s\{2,\}/\|/g' | grep -o -E "(?<=|)[^|]+" | head -1)

## Support manual SSID entry.
if [ "$CHENTRY" = "manually connect to a network" ] ; then
	MSSID=$(echo "Enter your network's SSID." | rofi -dmenu -p "SSID: " -font "$FONT" -lines 1 -normal-window -steal-focus)
	WIFI_PASS=$(echo "Enter the network password." | rofi -dmenu -password -p "Password: " -lines 1 -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH" -normal-window -steal-focus)
...
## Support connecting to the chosen network.
  ...
  iwctl station $DEVICE connect $CHSSID --dont-ask && exit
	WIFI_PASS=$(echo "Enter the network password." | rofi -dmenu -password -p "Password: " -lines 1 -location "$POSITION" -yoffset "$Y_OFF" -xoffset "$X_OFF" -font "$FONT" -width -"$R_WIDTH" -normal-window -steal-focus)
	iwctl station "$DEVICE" disconnec-normal-window -steal-focust
